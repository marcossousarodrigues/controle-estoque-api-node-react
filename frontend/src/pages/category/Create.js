...
function Create() {
  ...
  const {httpRequest, category} = useCategory();
  ...
  const handleSubmit = async (e) =>
  {
    ...
    setTimeout( async () => {
      await httpRequest('POST', category);
 ...
 const categoryFilter = category ? category.filter(obj => obj.id.toString().includes(idPesquisado)) : '';
 ...
 const categoryOrders = [...categoryFilter].sort((a, b) => {
  ...
  return (
      ...
      <h1>Categoria do Produto - Cadastrar</h1>
      ...
      <section className="container mt-2 mb-5" style={{ overflowX: 'auto' }}>
        <table className="w-100">
          ...
          {categoryOrders &&  <tbody>
              {categoryOrders && categoryOrders.map((obj, index) => (
